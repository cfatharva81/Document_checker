...
```mermaid
flowchart TD
    A[Filename] --> C[Normalize both sides]
    B[Metadata title and first Heading 1] --> C
    C --> D{Exact match?}
    D -- Yes --> P([PASS])
    D -- No --> E{Numbers conflict?}
    E -- Yes --> F([FAIL])
    E -- No --> G{Fuzzy similarity high?}
    G -- Yes --> P
    G -- No --> F
```
...
```mermaid
flowchart TD
    A[Metadata and info tables] --> C[Collect author candidates]
    B[Body and revision history] --> C
    C --> D[Drop generic and default names]
    D --> E{Valid author left?}
    E -- Yes --> P([PASS])
    E -- No --> F([FAIL])
```
...
```mermaid
flowchart TD
    A[Find revision table] --> B[Locate Date column]
    B --> C[Parse date values]
    C --> D{Dates readable?}
    D -- No --> F([FAIL])
    D -- Yes --> E{Not future and in order?}
    E -- Yes --> P([PASS])
    E -- No --> F
```
...
```mermaid
flowchart TD
    A[Metadata] --> D[Search for version patterns]
    B[Body, header, footer] --> D
    C[Tables and revision history] --> D
    D --> E{Version found?}
    E -- Yes --> P([PASS])
    E -- No --> F([FAIL])
```
...
```mermaid
flowchart TD
    A[Find revision heading] --> B{Heading found?}
    B -- Yes --> C[Tables below the heading]
    B -- No --> D[Fallback revision-table scan]
    C --> E{Table populated?}
    D --> E
    E -- Yes --> P([PASS])
    E -- No --> F([FAIL])
...
```mermaid
flowchart TD
    A[Signature paragraphs] --> C[Check structure]
    B[Signature tables] --> C
    C --> D{Signature table with headers and rows?}
    D -- Yes --> P([PASS])
    D -- No --> E{Structured signature label?}
    E -- Yes --> P
    E -- No --> I([IGNORE])
```
...
```mermaid
flowchart TD
    A[Find signature blocks] --> B{Date in same line or table row?}
    B -- Yes --> P([PASS])
    B -- No --> C[Check nearby paragraphs]
...
```mermaid
flowchart TD
    A[Collect body paragraphs] --> B[Exclude titles, headings, lists, tables]
    B --> C[Find dominant font and spacing]
    C --> D[Compare each paragraph]
    D --> E{Within tolerance?}
    E -- Yes --> P([PASS])
    E -- No --> F([FAIL with locators])
```
...
```mermaid
flowchart TD
    A[Extract document text] --> B[Run language checker]
    B --> C[Filter domain terms and identifiers]
    C --> D[Remove duplicates]
    D --> E{Genuine errors left?}
    E -- No --> P([PASS])
    E -- Yes --> F([FAIL])
```
...
```mermaid
flowchart TD
    A{Required sections configured?}
    A -- No --> N([NOT EVALUATED])
    A -- Yes --> B[Iterate headings]
    B --> C[Normalize heading text]
    C --> D{All sections found?}
    D -- Yes --> P([PASS])
    D -- No --> F([FAIL with missing list])
```
...
```mermaid
flowchart TD
    A[Inspect footers] --> B{Dynamic PAGE field?}
    B -- Yes --> P([PASS])
    B -- No --> C{Hardcoded number in footer?}
    C -- Yes --> F([FAIL — likely hardcoded])
    C -- No --> G([FAIL — no page number])
```
...
```mermaid
flowchart TD
    A[Extract body text] --> B[Exclude headings]
    B --> C[Flesch Reading Ease]
    B --> D[Gunning Fog]
    C --> E{Within thresholds?}
    D --> E
    E -- Yes --> P([PASS])
    E -- No --> F([FAIL with scores])
```
...
```mermaid
flowchart TD
    A[Inspect footers] --> B{Document ID?}
    B -- No --> F([FAIL with missing items])
    B -- Yes --> C{Confidentiality info?}
    C -- No --> F
    C -- Yes --> D{Page number?}
    D -- No --> F
    D -- Yes --> P([PASS])
